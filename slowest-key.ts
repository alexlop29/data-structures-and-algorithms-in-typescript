...
function slowestKey(releaseTimes: number[], keysPressed: string): string {
    const keysPressedArray = keysPressed.split("");
    let slowestKey = keysPressedArray[0];
    let slowestTime = releaseTimes[0];
    let length = (releaseTimes.length) - 1;
    let index = 1;

    while (index <= length){
        let lastTime = releaseTimes[index-1];
        let currTime = releaseTimes[index];
        let duration = currTime - lastTime;

        if (duration > slowestTime){
            slowestKey = keysPressedArray[index];
            slowestTime = duration;
        } else if (duration == slowestTime) {
            keysPressed[index] > slowestKey ? slowestKey = keysPressed[index] : '';
        }
