...
function sortTheStudents(score: number[][], k: number): number[][] {
    ...
    let retrievedValuePerStudent = [];
    score.forEach((value, index) => {
        retrievedValuePerStudent.push({
            index: index,
            value: value[k]
        });
    })

    let temp = retrievedValuePerStudent.sort(((n1,n2) => n1.value - n2.value));
    temp.reverse();

    temp.forEach((value) => {
        sortedByK.push(score[value.index]);
    })
